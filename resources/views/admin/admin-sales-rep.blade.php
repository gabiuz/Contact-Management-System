<x-admin-layout>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <h2 class="text-xl sm:text-2xl font-semibold text-gray-900">Sales Rep</h2>
      <div class="relative">
        <form method="GET" action="{{ route('admin-sales-rep') }}" class="relative">
      ...
      <button onclick="openAddSalesRepModal()"
        ...
        <span class="hidden sm:inline">Add Sales Rep</span>
        ...
        <table class="w-full border-collapse">
          ...
          <tbody>
            ...
            <tr class="bg-violet-50 border-8 border-white">
              <td class="px-3 sm:px-6 py-4 text-xs sm:text-sm text-center text-slate-500">{{ $rep->representative_id }}</td>
              <td class="px-3 sm:px-6 py-4 text-xs sm:text-sm text-center text-slate-500">{{ $rep->first_name }}</td>
              <td class="px-3 sm:px-6 py-4 text-xs sm:text-sm text-center text-slate-500 hidden md:table-cell">{{ $rep->middle_name }}</td>
              <td class="px-3 sm:px-6 py-4 text-xs sm:text-sm text-center text-slate-500">{{ $rep->last_name }}</td>
              <td class="px-3 sm:px-6 py-4 text-xs sm:text-sm text-center text-slate-500 hidden lg:table-cell">{{ $rep->email }}</td>
              <td class="px-6 py-4 text-sm text-center">
                <div class="flex items-center justify-center gap-2">
                  <button type="button"
                    onclick='openEditSalesRepModal(@json($rep), @json(route("admin-sales-rep.update", $rep)))'
                  ...
                  <button onclick="openDeleteConfirmModal(@js(route('admin-sales-rep.destroy', $rep)))"
            ...
            @empty
            <tr>
              <td colspan="6" class="px-6 py-4 text-sm text-center text-slate-500">No sales representatives found.</td>
            </tr>
            @endforelse
  ...
  <x-add-sales-rep-modal />
  <x-edit-sales-rep-modal />
</x-admin-layout>